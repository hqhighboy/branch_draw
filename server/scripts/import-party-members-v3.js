 ...
 * 导入党员信息脚本 V3
 * 
 * 此脚本用于导入党员信息Excel文件到数据库，并按照员工姓名为准补充到员工表中
 */
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        if (employees.length === 0) {
          throw new Error(`找不到员工: ${record['姓名']}`);
        }
        ...
        const [employeeInfo] = await conn.query(
          'SELECT department_path FROM employees WHERE id = ?',
          [employeeId]
        );
        
        const departmentPath = employeeInfo[0]?.department_path || null;
        
        // 添加到branch_personnel表
        await conn.query(
          `INSERT INTO branch_personnel (branch_id, name, department_path)
           VALUES (?, ?, ?)`,
          [branchId, record['姓名'], departmentPath]
        );
